...
if [ "$1" = 'default' ]; then
  ...
  if [ -n "${TAIGA_BACKEND_POSTGRESQL_HOST}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_POSTGRESQL_HOST'$^\1 = '${TAIGA_BACKEND_POSTGRESQL_HOST}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_POSTGRESQL_PORT}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_POSTGRESQL_PORT'$^\1 = '${TAIGA_BACKEND_POSTGRESQL_PORT}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_POSTGRESQL_NAME}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_POSTGRESQL_NAME'$^\1 = '${TAIGA_BACKEND_POSTGRESQL_NAME}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_POSTGRESQL_USER}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_POSTGRESQL_USER'$^\1 = '${TAIGA_BACKEND_POSTGRESQL_USER}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_POSTGRESQL_PASS}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_POSTGRESQL_PASS'$^\1 = '${TAIGA_BACKEND_POSTGRESQL_PASS}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_SITES_API_SCHEME}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_SITES_API_SCHEME'$^\1 = '${TAIGA_BACKEND_SITES_API_SCHEME}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_SITES_API_DOMAIN}" -a -n "${TAIGA_BACKEND_SITES_API_PORT}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_SITES_API_DOMAIN:TAIGA_BACKEND_SITES_API_PORT'$^\1 = '${TAIGA_BACKEND_SITES_API_DOMAIN}:${TAIGA_BACKEND_SITES_API_PORT}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_SITES_FRONT_SCHEME}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_SITES_FRONT_SCHEME'$^\1 = '${TAIGA_BACKEND_SITES_FRONT_SCHEME}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_SITES_FRONT_DOMAIN}" -a -n "${TAIGA_BACKEND_SITES_FRONT_PORT}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_SITES_FRONT_DOMAIN:TAIGA_BACKEND_SITES_FRONT_PORT'$^\1 = '${TAIGA_BACKEND_SITES_FRONT_DOMAIN}:${TAIGA_BACKEND_SITES_FRONT_PORT}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_MEDIA_URL}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_MEDIA_URL'$^\1 = '${TAIGA_BACKEND_MEDIA_URL}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_STATIC_URL}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_STATIC_URL'$^\1 = '${TAIGA_BACKEND_STATIC_URL}'^" local.py
  else
  ...
  if [ -n "${TAIGA_BACKEND_SECRET_KEY}" ]; then
    ...
    sed -i -e "s^# \(.*\) = 'TAIGA_BACKEND_SECRET_KEY'$^\1 = '${TAIGA_BACKEND_SECRET_KEY}'^" local.py
  else
  ...
  if [ "x${TAIGA_BACKEND_DEBUG}" = 'xTrue' -o "x${TAIGA_BACKEND_DEBUG}" = 'xFalse' ]; then
    ...
    echo ""                                   >> local.py
    echo "DEBUG = \"${TAIGA_BACKEND_DEBUG}\"" >> local.py
  fi
  ...
  if [ "x${TAIGA_BACKEND_CELERY_ENABLED}" = 'xTrue' -o "x${TAIGA_BACKEND_CELERY_ENABLED}" = 'xFalse' ]; then
    ...
    echo ""                                                     >> local.py
    echo "CELERY_ENABLED = \"${TAIGA_BACKEND_CELERY_ENABLED}\"" >> local.py
  fi
  ...
  if [ -n "${TAIGA_BACKEND_ADMIN_NAME}" -a -n "${TAIGA_BACKEND_ADMIN_EMAIL}" ]; then
    ...
    echo ""                                                                     >> local.py
    echo "ADMINS = ("                                                           >> local.py
    echo "    ('${TAIGA_BACKEND_ADMIN_NAME}', '${TAIGA_BACKEND_ADMIN_EMAIL}')," >> local.py
    echo ")"                                                                    >> local.py
  fi
