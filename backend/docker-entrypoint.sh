...
if [ "$1" = 'default' ]; then
  ...
  # mkdir -p gunicorn
  # echo '#!/bin/sh'                                                                              >  gunicorn/run
  # echo 'cd /taiga-backend'                                                                      >> gunicorn/run
  # echo 'export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'               >> gunicorn/run
  # echo "export PYTHONPATH=.:/usr/local/lib/python${PYTHON_MEJOR}.${PYTHON_MINOR}/site-packages" >> gunicorn/run
  # echo 'exec 2>&1'                                                                              >> gunicorn/run
  # echo 'exec gunicorn -w GUNICORN_WORKER -t GUNICORN_TIMEOUT -b 0.0.0.0:8080 taiga.wsgi'        >> gunicorn/run
  # chmod 0755 gunicorn/run
